...
class EvaluationService
{
    ...
    public function applyFiltersToExport($filters = [])
    {
    ...
    {
        $results = (clone $this->baseQuery)->select(
            'instructor_id',
            'instructor_name',
            'evaluation_id',
            'evaluation_title',
        ...
        $overall = (clone $this->baseQuery)
            ->select(
                'instructor_name',
                DB::raw('AVG(answer / evaluation_type * 5) as overall_rate')
            )
        ...
        $questions = $results
            ->pluck('evaluation_title')
            ->unique()
            ->values();

        $grandTotalQuestions = (clone $this->baseQuery)
            ->select(
                'evaluation_title',
                DB::raw('AVG(answer / evaluation_type * 5) as avg_rate')
            )
        ...
        $grandTotalOverall = (clone $this->baseQuery)
            ->select(DB::raw('AVG(answer / evaluation_type * 5) as overall_rate'))
            ->value('overall_rate');

        $grandTotal = [
            'questions' => [],
            'overall'   => round($grandTotalOverall, 2),
        ];
    ...
    // الفانكشن الجديدة - متوسط التقييم لكل محاضر لكل كاتيجوري
    public function getCategoryEvaluationData()
    {
        $avgPerInstructorCategory = (clone $this->baseQuery)
            ->select(
        ...
        $avgPerInstructorCategoryGrouped = $avgPerInstructorCategory
            ->groupBy('evaluation_category_name')
            ->map(function ($items, $categoryName) {
                return [
                    'evaluation_category_name' => $categoryName,
                    'instructors' => $items->map(function ($item) {
                        return [
        ...
        return [
            'categories' => $avgPerInstructorCategoryGrouped,
