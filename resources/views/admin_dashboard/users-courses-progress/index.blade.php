@extends('admin_dashboard.layout.master')
@section('Page_Title')  نسب التقدم للمستخدمين @endsection

@section('content')


    <div class="card">
        <div class="card-body">

            <div class="d-flex align-items-center justify-content-between">
                <h5 class="mb-0"> <i class="bi bi-grid-fill"></i>  نسب التقدم للمستخدمين </h5>
                <a href="{{route('admin.users.courses.progress.export', request()->all())}}" class="btn btn-success">
                    <i class="lni lni-download mx-1"></i> تحميل البيانات
                </a>
            </div>

            <div class="mt-5 mb-2 row">

                <form method="GET" action="" class="col-12">
                    <div class="row m-0 align-items-end">
                        <div class="col-md-3 mb-2">
                            <label for="course">الدورة التدريبية:</label>
                            <select name="course_id" id="course_id" class="form-control select2">
                                <option value="">كل الدورات التدريبية</option>
                                @foreach($allCourses as $id => $title)
                                    <option value="{{ $id }}" {{ $selectedCourse == $id ? 'selected' : '' }}>
                                        {{ $title }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3 mb-2">
                            <label for="group_id">المجموعة:</label>
                            <select name="group_id" id="group_id"  class="form-control select2">
                                @if(!is_null(request('course_id')) && !is_null(request('group_id')))
                                    @php
                                        $groups = \App\Models\Course::with('sections')->whereId(request('course_id'))->first();
                                    @endphp
                                    @foreach($groups->sections as $group)
                                        <option value="{{$group->id}}" @selected($group->id == request('group_id'))>
                                            {{$group->name}}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        <div class="col-md-3 mb-2">
                            <label for="user">الموظف:</label>
                            <select name="user_id"  id="allUsers" class="form-control">
                            </select>
                        </div>
                        <div class="col-md-3 mb-2">
                            <button type="submit" class="btn btn-main">فلتر</button>
                            <button type="reset" onclick="location.href='{{route('admin.users-courses-progress.index')}}'" class="mx-2 btn btn-danger">إلغاء</button>
                        </div>



                    </div>
                </form>


            </div>

            <div class="table-responsive mt-4">
                <table class="table align-middle table-hover text-center">
                    <thead class="table-secondary">
                    <tr>
                        <th>كود الموظف</th>
                        <th>الموظف</th>
                        <th>القسم</th>
                        <th>الدورة التدريبية</th>
                        <th>الجروب</th>
                        <th>نتيجة الأختبار النهائي</th>
                        <th>نسبة التقدم </th>
                    </tr>
                    </thead>
                    <tbody>
                    @forelse($content as $con)
                        <tr>
                            <td>{{$con['machine_code']}}</td>
                            <td>{{$con['user']}}</td>
                            <td>{{$con['department_name']}}</td>
                            <td>{{$con['course']}} {{ $con['course_type'] == 'offline' ? '🔴' : '🟢' }}</td>
                            <td>{{$con['group']}}</td>
                            <td>
                                @if(!is_null($con['total_degree']))
                                <div class="circle" style="margin: 0 50px;background: #f37021;border-radius: 50px;color: #fff9f5;">
                                    <strong>{{$con['user_degree']}}</strong>
                                    <hr style="margin: 2px 0 !important;opacity: 1">
                                    <strong>{{$con['total_degree']}}</strong>
                                </div>
                               @endif
                            </td>
                            <td>
                                <div class="progress" style="height:18px;">
                                    <div class="progress-bar progress-bar-striped bg-success" role="progressbar"
                                         style="width: {{ $con['progress'] }}%"
                                         aria-valuenow="{{ $con['progress'] }}" aria-valuemin="0" aria-valuemax="100">
                                    </div>
                                    <span class="percentage">{{ $con['progress'] }}%</span>
                                </div>
                            </td>

                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center">
                                <p>لا يوجد بيانات</p>
                            </td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
                <div>
                    {{ $content->links() }}
                </div>
            </div>
        </div>
    </div>


@endsection

@push('js')
    @include('admin_dashboard.components.delete')
    <script>
        $(document).on('change', '#course_id', function (){
            var course_id = $(this).val();
            $.ajax({
                url: "{{route('admin.get-groups-of-course')}}",
                type: 'GET',
                data: {
                    'course_id' : course_id
                },
                success: function(response) {
                    if(response.status)
                    {
                        $('#group_id').html(response.data.html);
                    }
                    else{
                        $('#group_id').html('');
                    }

                }
            })
        });
    </script>
@endpush
